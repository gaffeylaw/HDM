	...
	$.getJSON(settings.url, {root: root}, function(response) {
		...
		function createNode(parent) {
            ...
            if(settings.show_checkbox) {
                if(this.checked) {
                    a_span = $("<a/>").attr("href", "javascript:void(0);").html("<input type='checkbox' checked='checked' name='ids' value='"+this.id+"'/><span class='"+ classes + "'>" + this.text + "</span>");
                }else{
                }

            }else{
                a_span = $("<a/>").attr("href", "javascript:void(0);").html("<span class='"+ classes + "'>" + this.text + "</span>");
            }
