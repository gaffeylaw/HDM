class Irm::GeneralCategoriesController < ApplicationController
  ...
  def get_data
    ...
    respond_to do |format|
      format.json  {render :json => to_jsonp(general_categories.to_grid_json([:segment1_name, :segment2_name, :segment3_name, :concat_segment_name, :status_code, :category_type_name], count)) }
    end
  end  
end
